...
upto 64 bits wide. In addition to the HARDWARE performance counters, a SBI
...
The `event_idx.type == 0x1` for HARDWARE GENERAL events. For HARDWARE GENERAL
event, all non-zero values of `event_info` are reserved for future use whereas
...
----
    enum sbi_pmu_hw_id {
        ...
        SBI_PMU_HW_MAX,                    /* non-SBI */
    };
----

=== PMU HARDWARE CACHE events

The `event_idx.type == 0x2` for HARDWARE CACHE events. For HARDWARE CACHE
event, all non-zero values of `event_info` are reserved for future use whereas
the `event_idx.code` is encoded as follows:
[source, C]
----
    ...
    enum sbi_pmu_hw_cache_op_id {
        ...
        SBI_PMU_HW_CACHE_OP_MAX,     /* non-SBI */
    };

    enum sbi_pmu_hw_cache_op_result_id {
        SBI_PMU_HW_CACHE_RESULT_ACCESS = 0,
        SBI_PMU_HW_CACHE_RESULT_MISS   = 1,
        SBI_PMU_HW_CACHE_RESULT_MAX,   /* non-SBI */
    };
----

=== PMU HARDWARE RAW events

The `event_idx.type == 0x3` for HARDWARE RAW events. For HARDWARE RAW event,
the `event_idx.code` should be zero and the `event_info` parameter passed to
`sbi_pmu_counter_config_matching()` call (described below) will have the
RAW event value to be programmed in the MHPMEVENTx CSR.

=== PMU SOFTWARE events

The `event_idx.type == 0xf` for SOFTWARE events. For SOFTWARE event, all
...
----
    enum sbi_pmu_sw_id {
        ...
        SBI_PMU_SW_SET_TIMER                 = 3,
        SBI_PMU_SW_IPI_SENT                  = 4,
        SBI_PMU_SW_IPI_RECEIVED              = 5,
        SBI_PMU_SW_FENCE_I_SENT              = 6,
        SBI_PMU_SW_FENCE_I_RECEIVED          = 7,
        SBI_PMU_SW_SFENCE_VMA_SENT           = 8,
        SBI_PMU_SW_SFENCE_VMA_RECEIVED       = 9,
        SBI_PMU_SW_SFENCE_VMA_ASID_SENT      = 10,
        SBI_PMU_SW_SFENCE_VMA_ASID_RECEIVED  = 11,
        SBI_PMU_SW_HFENCE_GVMA_SENT          = 12,
        SBI_PMU_SW_HFENCE_GVMA_RECEIVED      = 13,
        SBI_PMU_SW_HFENCE_GVMA_VMID_SENT     = 14,
        SBI_PMU_SW_HFENCE_GVMA_VMID_RECEIVED = 15,
        SBI_PMU_SW_HFENCE_VVMA_SENT          = 16,
        SBI_PMU_SW_HFENCE_VVMA_RECEIVED      = 17,
        SBI_PMU_SW_HFENCE_VVMA_ASID_SENT     = 18,
        SBI_PMU_SW_HFENCE_VVMA_ASID_RECEIVED = 19,
        SBI_PMU_SW_MAX,                   /* non-SBI */
    };
----
...
This SBI call provides details about the specified counter such as underlying
CSR number, width of the counter, type of counter HARDWARE/SOFTWARE, etc.

The `counter_info` returned by this SBI call is encoded as follows:
[source, C]
----
    counter_info[11:0] = CSR (12bit CSR number)
    counter_info[19:12] = Width (number of bits)
...
This SBI call will find and configure a counter from a set of counters
which can monitor the specified event. The `counter_idx_base` and
`counter_idx_mask` parameters represent the set of counters whereas
...
This SBI calls provides the current value of a SOFTWARE counter.
...
This SBI call will start/enable the specified counter on the calling HART
...
This SBI call will stop/disable the specified counter on the calling HART.
