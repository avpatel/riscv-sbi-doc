...
NOTE: The `trig_max` may vary accross harts on a platform with asymmetric harts.
...
. Map an unused HW debug trigger which matches the trigger configuaration to an
. an unused `trig_idx`.
